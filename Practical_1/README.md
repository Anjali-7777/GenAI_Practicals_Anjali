...
An interactive **Streamlit** dashboard for exploring the **Natural Language Processing (NLP) pipeline** using **NLTK** and **spaCy**.

The application allows users to enter text and perform different NLP operations through an interactive and user-friendly interface.

Developed by **Anjali Patalbansi**.
...
The project is implemented in a **Google Colab Notebook** and uses the `%%writefile` command to generate the Streamlit application (`nlp_app.py`).
